...
class vehiculo
{
    ...
    public function InsertarVehiculoParametros()
	{
            ...
            $consulta =$objetoAccesoDato->RetornarConsulta("INSERT into vehiculos (patente,tipo,marca,color,estado)values(:patente,:tipo,:marca,:color,:estado)");
	...
	{
			$objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
			$consulta =$objetoAccesoDato->RetornarConsulta("SELECT * from vehiculos where patente = '$patenteAux'");
			$consulta->execute();
    ...
    public static function TraerTodoLosDisponibles()
	{
			$objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
			$consulta =$objetoAccesoDato->RetornarConsulta("SELECT * FROM `vehiculos` WHERE `estado`='libre'");
            $consulta->execute();	
            if($consulta->rowCount() == 0){
                return false;   
            }			
    ...
    public static function CambiarEstadoVehiculo($auxPatente,$auxEST)
    {
        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso();

        $consulta =$objetoAccesoDato->RetornarConsulta("UPDATE vehiculos set estado=:estado where patente='$auxPatente'");
    ...
    public static function listaFacturados()
    {
        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso();
        $consulta =$objetoAccesoDato->RetornarConsulta("SELECT id,patente,marca,color,idEmpleadoIngreso ,idCochera ,fechaHoraIngreso, idEmpleadoSalida , fechaHoraSalida, tiempoTrans , foto ,importe FROM `estacionados` WHERE fechaHoraSalida is not NULL");
        $consulta->execute();
        $vehiculoAux = $consulta->fetchAll(PDO::FETCH_CLASS);
        if($consulta->rowCount() == 0){
            return false;   
